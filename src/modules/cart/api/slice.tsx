...
export const selectItemCountById = (id: string) =>
    createSelector(selectCartItem, (items) => {
        const item = items.find((item: any) => item._id === id)
        return item ? item.quantity : 0
    })


export const selectTotalItemsInCart = createSelector(selectCartItem, (item) => {
    return item.reduce((total, item) => total + item.quantity, 0)
})

export const selectTotalCartPrice = createSelector(selectCartItem, (item) => {
    return item.reduce((total, item) => total + item.totalPrice, 0)
})
